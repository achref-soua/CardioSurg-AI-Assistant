...
from rag.prompt_templates import INTRAOP_SYSTEM_PROMPT
from langchain.schema import HumanMessage, SystemMessage


class IntraOpAgent(BaseAgent):
    def __init__(self):
        super().__init__(INTRAOP_SYSTEM_PROMPT)

    def generate_response(self, query: str, patient_id: str = None) -> str:
        # Build context based on query
        collections = ["patients", "devices", "guidelines", "notes"]
        if patient_id:
